...
class Display_Results():
        ...
        for filename, df in all_results_dict.items():
            ...
            for i, col in enumerate(cols):
                ...
                if i < 4:
                    ...
                    column_config[col] = st.column_config.Column(col, width=300)
                else:
                    column_config[col] = st.column_config.Column(col, width=150)
            ...
            # Round numeric values 
            replicate_cols = ["Standard Deviation (ppb)", "[PFAS](ppb)", "% RSD"]
            replicates_df[replicate_cols] = replicates_df[replicate_cols].round(2)
            
            # Replace None/NaN in the PFAS columns with a friendly label
            for col in ["Standard Deviation (ppb)", "[PFAS](ppb)", "% RSD"]:
    ...
    def download_results(self, 
        ...
        ppb_csv_bytes = combined_ppb_df.to_csv(index=False).encode("utf-8")
        
        # For the replicates table
        ...
        replicates_csv_bytes = combined_replicates_df.to_csv(index=False).encode("utf-8")
